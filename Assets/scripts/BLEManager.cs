...
public class BLEManager : MonoBehaviour
{
    ...
    // REMOVED: public UIManager uiManager; // UIManager is now a persistent singleton accessed via Instance
    ...
    // REMOVED: OnEnable and OnDisable methods for SceneManager.sceneLoaded subscription
    // REMOVED: OnSceneLoaded method for finding UIManager per scene

    void Start()
    {
        // In Start, we only need to initialize BLE and start scanning if not already connected.
        // The UIManager is now accessed directly via UIManager.Instance.
        ...
        if (!isConnected) // Only initialize and scan if not already connected from a previous session
        {
            ...
            BluetoothLEHardwareInterface.Initialize(true, false, () =>
            {
                ...
                {
                    UIManager.Instance.UpdateConnectionStatus(isConnected); // Update UI
                    UIManager.Instance.UpdateMessage("BLE Initialized. Scanning..."); // Show message
                }
                else
                {
                    Debug.LogWarning("UIManager instance not found. UI updates will not be available.");
                }
            ...
            (error) =>
            {
                ...
                // UPDATED: Access UIManager via Instance
                if (UIManager.Instance != null) // Try to update UI if UIManager is available
                {
                    UIManager.Instance.UpdateConnectionStatus(isConnected); // Update UI
                    UIManager.Instance.UpdateMessage("BLE Init Error: " + error); // Show error message
                }
            });
        }
        else
        {
             ...
             if (UIManager.Instance != null)
             {
                 UIManager.Instance.UpdateConnectionStatus(isConnected);
                 UIManager.Instance.UpdateMessage($"Already Connected. Pot: {latestPotValue:F2}, FSR: {latestFsrValue:F2}, ToF: {latestTofValue:F2}");
             }
        }
    }
    ...
    void ScanForDevice()
    {
         ...
         if (UIManager.Instance != null)
         {
             UIManager.Instance.UpdateMessage("Scanning..."); // Update UI message
         }
    ...
    void ConnectToDevice(string address)
    {
        Debug.Log("Attempting to connect to " + address);
         ...
         if (UIManager.Instance != null)
         {
             UIManager.Instance.UpdateMessage("Connecting..."); // Update UI message
         }
        ...
        BluetoothLEHardwareInterface.ConnectToPeripheral(
            ...
            (addressCallback, serviceUUIDCallback, characteristicUUIDCallback) => // Action<string, string, string>
            {
                ...
                if (!isConnected) // Only process connection logic the first time this callback is hit for a new connection
                {
                    ...
                    {
                        UIManager.Instance.UpdateConnectionStatus(isConnected); // Update UI
                        UIManager.Instance.UpdateMessage("Connected. Subscribing..."); // Update UI message
                    }
            ...
            (error) => // Action<string>
            {
                ...
                if (UIManager.Instance != null) // Try to update UI if UIManager is available
                {
                    UIManager.Instance.UpdateConnectionStatus(isConnected); // Update UI (assuming error means disconnected)
                    UIManager.Instance.UpdateMessage("BLE Error: " + error); // Show error message
                }
    ...
    {
         // Assuming the data is an array of 3 floats (FSR, Pot, ToF) sent as bytes from ESP32
        ...
        if (bytes != null && bytes.Length >= 12) // 3 values * 4 bytes/value = 12 bytes minimum
        {
             try
             {
                // CORRECTED: Order to match ESP32 data packing: FSR, Pot, ToF
                ...
                // Update UI message with the latest values (if UIManager is set up for this)
                 // UPDATED: Access UIManager via Instance
                 if (UIManager.Instance != null && UIManager.Instance.messageText != null)
                 {
                      UIManager.Instance.UpdateMessage($"Pot: {latestPotValue:F2}, FSR: {latestFsrValue:F2}, ToF: {latestTofValue:F2}");
                 }
                 // Also update debug text if available
                 // UPDATED: Access UIManager via Instance
                 if (UIManager.Instance != null && UIManager.Instance.debugText != null)
                 {
                     // You can put specific debug info here, or just echo the message
                      UIManager.Instance.UpdateDebug($"Raw Data: {BitConverter.ToString(bytes)}");
                 }
             ...
             catch (Exception ex)
             {
                 Debug.LogError($"Error processing received BLE data: {ex.Message}");
                  ...
                  if (UIManager.Instance != null && UIManager.Instance.debugText != null)
                  {
                     UIManager.Instance.UpdateDebug($"Data processing error: {ex.Message}");
                  }
             }
        }
        else
        {
            ...
            Debug.LogWarning($"Received data bytes length ({bytes?.Length ?? 0}) is not as expected for 3 values (expected >= 12 bytes), or bytes is null.");
             ...
             if (UIManager.Instance != null && UIManager.Instance.debugText != null)
             {
                  UIManager.Instance.UpdateDebug($"Received unexpected data format. Length: {bytes?.Length ?? 0}");
             }
        }
    }
...
//old script ----------------------------------- (not constant ui manager reference)
// using System;
// using TMPro;
// using UnityEngine;
// using UnityEngine.SceneManagement; // Required for SceneManager
